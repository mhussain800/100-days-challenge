...
export default function App() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(sessionsRef, (snapshot) => {
      ...
      console.error('Study sessions sync error:', error);
      showSaveStatus('Study sync unavailable');
    });
  }, [user]);
  ...
  const saveStudySettings = ({ subjects: nextStudySubjects, sources: nextStudySources, weeklyGoals: nextWeeklyStudyGoals }) => savePreferences({
    nextStudySubjects,
    nextStudySources,
    nextWeeklyStudyGoals,
  });
  const addStudySubject = async (name) => {
    const cleanName = name.trim();
    if (!cleanName) throw new Error('Enter a subject name.');
    if (studySubjects.some((subject) => subject.name.toLocaleLowerCase() === cleanName.toLocaleLowerCase())) {
  ...
  const syncStudyHabitForDates = async (nextSessions, dateKeys) => {
    ...
    dateKeys.forEach((dateKey) => {
      const totalMinutes = nextSessions
      ...
      const currentDay = nextLogs[dateKey] || {};
      nextLogs[dateKey] = {
        ...currentDay,
        study: { ...(currentDay.study || {}), checked: totalMinutes > 0, val: hours },
        timestamp: Date.now(),
      };
    });
    setLogs(nextLogs);
    const logsRef = doc(db, 'users', user.uid, 'challenge_data', 'logs');
    await setDoc(logsRef, { records: nextLogs }, { merge: true });
  };
  ...
  const saveStudySession = async (session) => {
    ...
    const sessionRef = session.id
      ? doc(db, 'users', user.uid, 'study_sessions', session.id)
      : doc(collection(db, 'users', user.uid, 'study_sessions'));
    ...
    const nextSessions = [...studySessions.filter((item) => item.id !== sessionRef.id), optimisticSession]
    ...
    try {
      await setDoc(sessionRef, {
        ...safeSession,
        studyDate: Timestamp.fromDate(parseDateKey(safeSession.dateKey)),
      ...
      try {
        ...
        console.error('Study habit sync error:', habitError);
      }
  ...
  const deleteStudySession = async (session) => {
    ...
    try {
      ...
      setStudySessions(studySessions);
      showSaveStatus('Could not delete');
      throw error;
    }
  };
